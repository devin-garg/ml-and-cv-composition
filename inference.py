...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while self.numParticles > len(self.particles):
            if len(self.legalPositions) < self.numParticles - len(self.particles):
                self.particles += self.legalPositions
            else:
                self.particles += self.legalPositions[0:(self.numParticles - len(self.particles))]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] += emissionModel[trueDistance] * beliefState[p]
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles:
            temporaryParticles.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, pos))))
        self.particles = temporaryParticles
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        myBeliefState = util.Counter()
        for particles in self.particles:
            myBeliefState[particles] += 1.0
        myBeliefState.normalize()
        return myBeliefState
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        myLegalPos = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(myLegalPos)
        self.particleList = []
        for particlePos in myLegalPos:
            if len(self.particleList) < self.numParticles:
                self.particleList.append(particlePos)
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        for particlePos in self.particleList:
            accumulator = 1.0
            for indexOfGhost in range(self.numGhosts):
                if noisyDistances[indexOfGhost] == None:
                    particlePos = self.getParticleWithGhostInJail(particlePos, indexOfGhost)
                else:
                    trueDistance = util.manhattanDistance(particlePos[indexOfGhost], pacmanPosition)
                    accumulator *= emissionModels[indexOfGhost][trueDistance]
            allPossible[particlePos] += accumulator
        if allPossible.totalCount() == 0:
            self.initializeParticles()
        else:
            allPossible.normalize()
