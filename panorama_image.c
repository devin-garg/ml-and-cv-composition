...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
		...
		float distanceFound = l1_distance(a[j].data, b[0].data, a[j].n);
		for(int k = 1; k < bn; k++){
			float distanceLocal = l1_distance(a[j].data, b[k].data, a[j].n);
			if (distanceLocal < distanceFound){
				distanceFound = distanceLocal;
				bind = k;
			}
		}
        m[j].ai = j;
        ...
        m[j].p = a[j].p;
        m[j].q = b[bind].p;
        m[j].distance = distanceFound; // <- should be the smallest L1 distance!
    }
...
point project_point(matrix H, point p)
{
	...
	c.data[1][0] = p.y;
	c.data[2][0] = 1.0;
	matrix multRes = matrix_mult_matrix(H, c);
    point q = make_point(multRes.data[0][0]/multRes.data[2][0], multRes.data[1][0]/multRes.data[2][0]);
...
matrix compute_homography(match *matches, int n)
{
    ...
    int i;
    for(i = 0; i < n; ++i){
        double px  = matches[i].p.x;
        double qx = matches[i].q.x;
        double py  = matches[i].p.y;
        double qy = matches[i].q.y;
...
image combine_images(image a, image b, matrix H)
{
    ...
    // estimate the value of b at that projection, then fill in image c.
        ...
        for(j = 0; j < c.h; ++j){
            for(i = 0; i < c.w; ++i){
				point testPoint = project_point(H, make_point(i+dx,j+dy));
				if(testPoint.x < b.w && testPoint.x > 0 && testPoint.y < b.h && testPoint.y > 0){
                    for(k = 0; k < c.c; ++k){
					   set_pixel(c, i, j, k, bilinear_interpolate(b, testPoint.x, testPoint.y, k));
                    }
				}
            }
        }
