...
matrix im2col(image im, int size, int stride)
{
    ...
    int cols = outw * outh;
    matrix column = make_matrix(rows, cols);
    ...
    for (int ch = 0; ch < im.c; ++ch){
      int sizeCaptured = ch*size*size*cols;
      for (int row = 0; row < im.h; row = row + stride) {
          for (int col = 0; col < im.w; col = col + stride) {
              int middleOf = col + row*im.w + ch*im.w*im.h;
              int changeSet = 0;
              for (int i = l; i <= r; i++) {
                for (int j = l; i <= r; i++) {
                  int srcind = middleOf + i*im.w + j;
                  int dest = sizeCaptured + changeSet*cols;
                  if(!(col+j >= im.w || row+i >= im.h || col+j < 0 || row+i < 0)){
                    column.data[dest] = im.data[srcind];
                  }
                  changeSet++;
                }
              }
...
void col2im(matrix column, int size, int stride, image im)
{
    int outw = (im.w-1)/stride + 1;
    int outh = (im.h-1)/stride + 1;
    int rows = im.c*size*size;
          ...
          for (int col = 0; col < im.w; col = col + stride) {
              ...
              for (int i = l; i <= r; i++) {
                for (int j = l; i <= r; i++) {
                  ...
                  int dest = sizeCaptured + changeSet*cols;
                  if(!(col+j >= im.w || row+i >= im.h || col+j < 0 || row+i < 0)){
                    im.data[srcind] += column.data[dest];
                  }
                  changeSet++;
                }
              }
