...
matrix variance(matrix x, matrix m, int spatial)
{
    ...
    for(i = 0; i < x.rows; ++i){
        for(j = 0; j < x.cols; ++j){
            float val = x.data[i*x.cols + j] - m.data[j/spatial];
            v.data[j/spatial] += val*val;
        }
    }
    for(i = 0; i < v.cols; ++i){
...
matrix normalize(matrix x, matrix m, matrix v, int spatial)
{
    ...
    for(i = 0; i < x.rows; ++i){
        for(j = 0; j < x.cols; ++j){
          norm.data[i*norm.cols + j] = (x.data[i*x.cols + j] - m.data[j/spatial]) / sqrt(v.data[j/spatial] + 0.001);
        }
    }
    return norm;
...
matrix delta_mean(matrix d, matrix variance, int spatial)
{
    ...
    for(i = 0; i < dm.cols; ++i){
        dm.data[i] *= -1.0/sqrt(variance.data[i] + 0.001);
    }
    return dm;
}
...
matrix delta_variance(matrix d, matrix x, matrix mean, matrix variance, int spatial)
{
    ...
    for(i = 0; i < d.rows; ++i){
        for(j = 0; j < d.cols; ++j){
          dv.data[j/spatial] += d.data[i*d.cols + j]*(x.data[i*x.cols + j] - mean.data[j/spatial]);
        }
    }
    for(i = 0; i < dv.cols; ++i){
        dv.data[i] *= (-1.0/2)*pow(variance.data[i] + 0.001, -3/2.0);
    }
    return dv;
}
...
matrix delta_batch_norm(matrix d, matrix dm, matrix dv, matrix mean, matrix variance, matrix x, int spatial)
{
    ...
    for(i = 0; i < d.rows; ++i){
        for(j = 0; j < d.cols; ++j){
          dx.data[i*dx.cols + j] = (d.data[i*d.cols + j]/sqrt(variance.data[j/spatial] + 0.001)) + dv.data[j/spatial] * (2.0/d.rows/spatial)*(x.data[i*x.cols + j] - mean.data[j/spatial]) + dm.data[j/spatial]*(1.0/d.rows/spatial);
        }
    }
    return dx;
}
